...
def LCS_DP_2(X, Y, m, n):
    ...
    lcs = lcs[::-1]
    # print("debugging",lcs)
    s = lcs  # your string here
    result = ""  # initialize an empty string to store the result

    for i in range(len(s)):
        if i == 0:  # skip the first character
            result += s[i]
        elif s[i] > s[i - 1]:  # add the character to the output only if it's greater than the previous character
            result += s[i]


    print("Length of Longest Common Subsequence is: ", len(result))
    print("The Longest Common Subsequence of " + f'"{X}"' + " and " + f'"{Y}"' + " is " + f'"{result}"')
